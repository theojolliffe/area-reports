<script>
    ...
    function results(place, dataSelect) {

        console.log('dataSelect', dataSelect)
        console.log('section', section)

        let dataSect = dataSelect[section]

        let perc= dataSect.label.split("_")[1]=='perc'

        let selectors = dataSect['label'].split("_");
        if (selectors[4]) {
        ...
        function nationComp(place, breaks, natRank, selectors, verb) {
            ...
            if (natRank<2) {
                return " is " + verb + " faster here than any other " + gssLookupShort[place.code.slice(0,3)]
            }
            if (natRank<4) {
                return " the " + ordinal_suffix_of(Math.abs(natRank)) + " fastest " + verb + " of any " + gssLookupShort[place.code.slice(0,3)]
            }
            else {
                return " is " + verb + " " + adjectify(place['data'][selectors[0]][selectors[1]+'_rank'][selectors[2]][selectors[3]], ['faster here', 'slower here'], breaks) + " the average"
            }
        }
        ...
        function overTake(place, selectors) {
            console.log('sellectors', selectors)
        ...
        let subj
        if (selectors[0]=="agemed") {
            if (!dataSelect.alt) {
                subj = "_70plus"
            } else { 
                subj = "_"+dataSelect.alt.split("_")[3] 
            }
        } else { subj = "_all"}
        ...
        let breaks = []; 
        for (let i=0; i<10; i++) {breaks.push(Math.round((i * ((place.type=="wd")?8056:336)) / 10))}

        let fasterSlower = nationComp(place, breaks, natRank, selectors, topic['verb']['positive']);
...
{#if load}
    {#each Object.values(results(place, dataSelect)) as sentence}
        <p>{sentence}</p>
    {/each}
{/if}
